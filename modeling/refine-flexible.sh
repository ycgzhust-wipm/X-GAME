...
protocol.initParams('${xplorhome}/parallhdg_new.pro')

protocol.initStruct('${proteinA}.psf',erase=False)
protocol.initCoords('${proteinA}.pdb')
AtomSel("all").apply( SetProperty('segmentName', 'ALT0') )


protocol.initStruct('${proteinB}.psf',erase=False)
protocol.initCoords('${proteinB}.pdb')
...
command("""
    ...
    inter = (segid ALT0 and resid ${domain1})(all and not (segid ALT0 and resid ${domain1}))
    inter = (segid ALT0 and resid ${domain2})(all and not (segid ALT0 and resid ${domain2}))
    inter = (segid ALT0 and resid ${loop1})(all)
    weights * 1 end end
...
dyn.fix("""segid BLT0  """)
dyn.fix(""" segid ALT0 and resid ${domain2}""")
dyn.group(""" segid ALT0 and resid ${domain1} """)
